...
public class GuiEnergyCellController extends BeefGuiBase {
	...
	//private BeefGuiFluidBar redstoneBar;
	
	public GuiEnergyCellController(Container container, TileEntityMultiblockPart tileEntityMultiblockPart) {
		super(container);
		
		ySize = 186;
		
		this.part = tileEntityMultiblockPart;
		this.cell = (MultiblockCell) part.getMaster();
	}
	...
	public void initGui() {
		...
		titleString = new BeefGuiLabel(this, "Energy Cell Control", leftX, topY);
		topY += titleString.getHeight() + 8;
	
		statusString = new BeefGuiLabel(this, "", leftX+1, topY);
		topY += statusString.getHeight() + 4;
		
		redstoneBar = new BeefGuiFluidBar(this, guiLeft + 24, guiTop + 42, cell, MultiblockCell.TANK_INPUT);
		powerBar = new BeefGuiPowerBar(this, guiLeft + 145, guiTop + 22, this.cell);
		 */
		//registerControl(titleString);
		//registerControl(statusString);
		//registerControl(redstoneBar);
	...
	protected void actionPerformed(GuiButton button) {
		if(button.id == 0 || button.id == 1) {
			...
			if(newSetting != cell.getActive()) {
				LogHelper.info("Sending Message: " + cell + " | " + newSetting);
                CommonPacketHandler.INSTANCE.sendToServer(new MultiblockCommandActivateMessage(cell, newSetting));
			}
		}
