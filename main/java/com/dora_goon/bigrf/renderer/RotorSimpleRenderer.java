...
public class RotorSimpleRenderer implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID,
		...
		if(metadata != BlockTurbineRotorPart.METADATA_BLADE) {
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z,
		...
		if(te instanceof TileEntityMultiblockRotorPart) {
			TileEntityMultiblockRotorPart rotorPart = (TileEntityMultiblockRotorPart)te;
			if (rotorPart.getMaster() instanceof MultiblockTurbine){
				MultiblockTurbine master = (MultiblockTurbine) rotorPart.getMaster();
				if(rotorPart.isConnected() && master.isAssembled() && master.getActive()) {
		...
		}else if(BlockTurbineRotorPart.isRotorBearing(metadata)){
			//TODO Bearing should not have blades
			ForgeDirection majorAxis = findRotorMajorAxis(world, x, y, z, block);
			boolean[] hasBlades = findBlades(world, x, y, z, block, majorAxis);
			renderRotorShaft(block, renderer, metadata, majorAxis, hasBlades, x, y, z, false);
		}else {
	...
	private void renderBladeFromWorld(RenderBlocks renderer, IBlockAccess world, int x, int y, int z, Block block, int metadata) {
		...
		if(te instanceof TileEntityMultiblockRotorPart) {
			TileEntityMultiblockRotorPart rotorPart = (TileEntityMultiblockRotorPart)te;
			if(rotorPart.isConnected() && rotorPart.getMaster().equals(MultiblockTurbine.class)) {
				rotorDir = ((MultiblockTurbine) rotorPart.getMaster()).getRotorDirection();
			}
		}
	...
	private static void renderRotorBladeConnection(RenderBlocks renderer, Block block, int metadata,
		...
		IIcon IIcon = Reference.blockTurbineRotorPart.getRotorConnectorIcon();
		for(int i = 0; i < 4; i++) {
