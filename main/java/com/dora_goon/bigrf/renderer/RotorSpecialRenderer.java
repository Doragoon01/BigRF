package com.dora_goon.bigrf.renderer;
...
public class RotorSpecialRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntityAt(TileEntity tileentity, double x, double y,
		...
		TileEntityMultiblockRotorBearing bearing = (TileEntityMultiblockRotorBearing)tileentity;
		
		if(bearing == null || !bearing.isConnected() || bearing.getMaster() != MultiblockTurbine.class) { return; }
		
		MultiblockTurbine turbine = (MultiblockTurbine) bearing.getMaster();
		
		if(!turbine.isAssembled() || !turbine.getActive()) { return; }
	...
	int generateRotor(RotorInfo rotorInfo) {
		...
		while(rotorIdx < rotorInfo.rotorLength) {
			...
			RotorSimpleRenderer.renderRotorShaft(Reference.blockTurbineRotorPart, renderBlocks, BlockTurbineRotorPart.METADATA_SHAFT, rotorDir, hasBlades, currentRotorCoord.x, currentRotorCoord.y, currentRotorCoord.z, false);
			for(int bladeIdx = 0; bladeIdx < bladeDirs.length; bladeIdx++) {
				...
				while(bladeLen < rotorInfo.bladeLengths[rotorIdx][bladeIdx]) {
					RotorSimpleRenderer.renderBlade(renderBlocks, bladeCoord.x, bladeCoord.y, bladeCoord.z, Reference.blockTurbineRotorPart, BlockTurbineRotorPart.METADATA_BLADE, rotorInfo.rotorDirection);
					bladeLen++;
					bladeCoord.translate(bladeDirs[bladeIdx]);
				}
			}
