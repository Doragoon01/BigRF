...
public abstract class RectangularMultiblockMasterBase extends
		MultiblockMasterBase {

	protected RectangularMultiblockMasterBase(World world) {
		super(world);
	}
	...
	protected void isMachineWhole() throws MultiblockValidationException {
		LogHelper.info("RMbMB - Checking if Multiblock is whole");
		...
		Class<? extends RectangularMultiblockMasterBase> myClass = this.getClass();
			...
			for(int y = minimumCoord.y; y <= maximumCoord.y; y++) {
				for(int z = minimumCoord.z; z <= maximumCoord.z; z++) {
					...
					if(te instanceof RectangularMultiblockTileEntityBase) {
						...
						if(!myClass.equals(part.getMultiblockMaster().getClass()))
						{
							throw new MultiblockValidationException(String.format("RMbMB - Part @ %d, %d, %d is incompatible with machines of type %s", x, y, z, myClass.getSimpleName()));
						}
					}
					else {
		...
		LogHelper.info("RMbMB - Multiblock is whole");
	}	
