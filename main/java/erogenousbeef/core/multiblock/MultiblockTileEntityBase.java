...
public abstract class MultiblockTileEntityBase extends IMultiblockPart {
	private MultiblockMasterBase master;
	...
	@Override
	public Set<MultiblockMasterBase> attachToNeighbors() {
		//LogHelper.info("MbTeB - " + this.getMultiblockMasterType().getSimpleName() + " is looking for a master");
		Set<MultiblockMasterBase> master = null;
		MultiblockMasterBase bestMaster = null;
		
		// Look for a compatible master in our neighboring parts.
		IMultiblockPart[] partsToCheck = getNeighboringParts();
		for(IMultiblockPart neighborPart : partsToCheck) {
			if(neighborPart.isConnected()) {
				MultiblockMasterBase candidate = neighborPart.getMultiblockMaster();
				if(!this.getMultiblockMasterType().equals(MultiblockBase.class)){
					if(candidate.getClass().equals(MultiblockBase.class)){
						LogHelper.info("MbTeB - Removing Base Multiblock");
						MultiblockRegistry.addDeadMaster(candidate.worldObj, candidate);
						continue;
					}
					if(!candidate.getClass().equals(this.getMultiblockMasterType())) {
						// Skip multiblocks with incompatible types
						//LogHelper.info("MbTeB - " + this.getMultiblockMasterType().getSimpleName() + " is not joining incompatible block: " + candidate.getClass().getSimpleName());
						continue;
					}
				}
				if(master == null) {
					master = new HashSet<MultiblockMasterBase>();
					bestMaster = candidate;
				}
				else if(!master.contains(candidate) && candidate.shouldConsume(bestMaster)) {
					bestMaster = candidate;
				}				
		...
		if(bestMaster != null) {
			...
			//LogHelper.info("MbTeB - Master found");
			this.master = bestMaster;
			bestMaster.attachBlock(this);
		}else {
			//LogHelper.info("MbTeB - No Master found");
			}
	...
	protected void encodeDescriptionPacket(NBTTagCompound packetData) {
		if(this.isMultiblockSaveDelegate() && isConnected()) {
			...
			getMultiblockMaster().formatDescriptionPacket(tag);
	...
	protected void decodeDescriptionPacket(NBTTagCompound packetData) {
		if(packetData.hasKey("multiblockData")) {
			...
			if(isConnected()) {
				getMultiblockMaster().decodeDescriptionPacket(tag);
			}
			else {
	...
	@Override
	public abstract void onMachineAssembled(MultiblockMasterBase multiblockMasterBase);
	...
	@Override
	public MultiblockMasterBase getMultiblockMaster() {
		return master;
	}
	...
	@Override
	public void onAssimilated(MultiblockMasterBase newMaster) {
	...
	@Override
	public void onDetached(MultiblockMasterBase oldMaster) {
		this.master = null;
	}

	@Override
	public abstract MultiblockMasterBase createNewMultiblock();
	...
	@Override
	public void onOrphaned(MultiblockMasterBase master, int oldSize, int newSize) {
		this.markDirty();
