 ...
 */
public abstract class MultiblockMasterBase implements IActivateable, IFluidHandler{
	...
	protected MultiblockMasterBase(World world) {
		// Multiblock stuff
		worldObj = world;
	...
	public void attachBlock(IMultiblockPart part) {
		//LogHelper.info("MbCBase - Attaching Block");
		...
		MultiblockRegistry.addDirtyMaster(worldObj, this);
		//LogHelper.info("Attached Block, " + part + ", to Multiblock");
	}
	...
	public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
		...
		MultiblockRegistry.addDirtyMaster(this.worldObj,  this);
	...
	private void assembleMachine(AssemblyState oldState) {
		//LogHelper.info("MbMB - Assembling Machine");
	 ...
	 */
	public void assimilate(MultiblockMasterBase other) {
		//LogHelper.info("MbMB - " + this.getClass().getSimpleName() + " is assimilating " + other.getClass().getSimpleName());
		CoordTriplet otherReferenceCoord = other.getReferenceCoord();
		if(!(other instanceof MultiblockBase)){
	 ...
	 * @param otherMaster The controller consuming this controller.
	 */
	private void _onAssimilated(MultiblockMasterBase otherMaster) {
		if(referenceCoord != null) {
	 ...
	 */
	protected abstract void onAssimilate(MultiblockMasterBase assimilated);
	 ...
	 */
	protected abstract void onAssimilated(MultiblockMasterBase assimilator);
	
	/**
	 * Driver for the update loop. If the machine is assembled, runs
	 * the game logic update method.
	 * @see erogenousbeef.core.multiblock.MultiblockMasterBase#update() //TODO Fix this Javadoc
	 */
	 ...
	 * @param otherMaster The other multiblock controller.
	 * @return True if this multiblock should consume the other, false otherwise.
	 */
	public boolean shouldConsume(MultiblockMasterBase otherMaster) {
		//LogHelper.info("MbMB - " + this + " is looking to consume " + otherMaster);
		
		if(!otherMaster.getClass().equals(getClass()) && !otherMaster.getClass().equals(MultiblockBase.class)) {
		...
		if(otherMaster == this) { return false; } // Don't be silly, don't eat yourself.
		if(!otherMaster.getClass().equals(getClass())){return true;} //always merge into non-base master
		
		int res = _shouldConsume(otherMaster);
		if(res < 0) { return true; }
			...
			auditParts();
			otherMaster.auditParts();
			
			res = _shouldConsume(otherMaster);
			if(res < 0) { return true; }
			else if(res > 0) { return false; }
			else {
				BeefCoreLog.error("My Controller (%d): size (%d), parts: %s", hashCode(), connectedParts.size(), getPartsListString());
				BeefCoreLog.error("Other Controller (%d): size (%d), coords: %s", otherMaster.hashCode(), otherMaster.connectedParts.size(), otherMaster.getPartsListString());
	...
	private int _shouldConsume(MultiblockMasterBase otherMaster) {
		CoordTriplet myCoord = getReferenceCoord();
		CoordTriplet theirCoord = otherMaster.getReferenceCoord();
	...
	public Set<IMultiblockPart> checkForDisconnections() {
		...
		if(this.isEmpty()) {
			MultiblockRegistry.addDeadMaster(worldObj, this);
			return null;
		}
		...
		while(!partsToCheck.isEmpty()) {
			...
			for(IMultiblockPart nearbyPart : nearbyParts) {
				...
				if(nearbyPart.getMultiblockMaster() != this) {
					continue;
				}
	 ...
	 * @see MultiblockMasterBase#markReferenceCoordForUpdate()
	 */
