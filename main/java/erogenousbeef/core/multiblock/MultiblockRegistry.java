...
public class MultiblockRegistry {
	 ...
	 * @param master The dirty controller 
	 */
	public static void addDirtyMaster(World world,
			MultiblockMasterBase master) {
		if(registries.containsKey(world)) {
			registries.get(world).addDirtyMaster(master);
		}
		else {
	 ...
	 * @param master The dead controller
	 */
	public static void addDeadMaster(World world, MultiblockMasterBase master) {
		if(registries.containsKey(world)) {
			registries.get(world).addDeadMaster(master);
		}
		else {
			BeefCoreLog.warning("Controller %d in world %s marked as dead, but that world is not tracked! Controller is being ignored.", master.hashCode(), world);
		}
	}
	 ...
	 */
	public static Set<MultiblockMasterBase> getMastersFromWorld(World world) {
		if(registries.containsKey(world)) {
			return registries.get(world).getMasters();
		}
		return null;
	}
