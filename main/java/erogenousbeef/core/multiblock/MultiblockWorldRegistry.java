...
public class MultiblockWorldRegistry {
	...
	private Set<MultiblockMasterBase> controllers;		// Active controllers
	private Set<MultiblockMasterBase> dirtyControllers;	// Controllers whose parts lists have changed
	private Set<MultiblockMasterBase> deadControllers;	// Controllers which are empty
	...
	public MultiblockWorldRegistry(World world) {
		...
		controllers = new HashSet<MultiblockMasterBase>();
		deadControllers = new HashSet<MultiblockMasterBase>();
		dirtyControllers = new HashSet<MultiblockMasterBase>();
	...
	public void tickStart() {
		if(controllers.size() > 0) {
			for(MultiblockMasterBase controller : controllers) {
	...
	public void processMultiblockChanges() {
		...
		List<Set<MultiblockMasterBase>> mergePools = null;
		if(orphanedParts.size() > 0) {
			...
			//LogHelper.info("MbWR - Orphans to Process: " + orphanedParts.size());
			if(orphansToProcess != null && orphansToProcess.size() > 0) {
				Set<MultiblockMasterBase> compatibleControllers;
				...
				for(IMultiblockPart orphan : orphansToProcess) {
					...
					//LogHelper.info("MbWR - Controllers to be Attached: " + compatibleControllers);
					if(compatibleControllers == null) {
						...
						//LogHelper.info("MbWR - Creating new Multiblock");
						MultiblockMasterBase newMaster = orphan.createNewMultiblock();
						newMaster.attachBlock(orphan);
						this.controllers.add(newMaster);
						// TODO non-dominant masters need to attatch here
						
					}
					else if(compatibleControllers.size() > 1) {
						if(mergePools == null) { mergePools = new ArrayList<Set<MultiblockMasterBase>>(); }
						...
						List<Set<MultiblockMasterBase>> candidatePools = new ArrayList<Set<MultiblockMasterBase>>();
						for(Set<MultiblockMasterBase> candidatePool : mergePools) {
						...
						if(candidatePools.size() <= 0) {
							...
							Set<MultiblockMasterBase> masterPool = candidatePools.get(0);
							Set<MultiblockMasterBase> consumedPool;
		...
		if(mergePools != null && mergePools.size() > 0) {
			//LogHelper.info("MbWR - Merge Pools: " + mergePools);
			...
			for(Set<MultiblockMasterBase> mergePool : mergePools) {
				// Search for the new master machine, which will take over all the blocks contained in the other machines
				MultiblockMasterBase newMaster = null;
				for(MultiblockMasterBase controller : mergePool) {
		...
		if(dirtyControllers.size() > 0) {
			...
			for(MultiblockMasterBase controller : dirtyControllers) {
		...
		if(deadControllers.size() > 0) {
			for(MultiblockMasterBase controller : deadControllers) {
	 ...
	 */
	public void addDeadMaster(MultiblockMasterBase deadController) {
	 ...
	 */
	public void addDirtyMaster(MultiblockMasterBase dirtyController) {
	 ...
	 */
	public Set<MultiblockMasterBase> getMasters() {
